...
**Status:** `done` — closed 2026-09-16 (session close-out). Both halves landed in
`tools/t137_m4_leg.sh`: (a) `export BH_GM_NAMES=t137a__00` (L55-56), (b) 240 s
abort-fast on missing "battle joined" (L63+). Proven by `logs/t137_m4e29b_server.log`:
battles=1, p99=2.8 ms, replay mm=0. M4 verdict itself stays FAIL-open on flips
(see `done/T-157.md` re-verdict + `T-157-F3`).
