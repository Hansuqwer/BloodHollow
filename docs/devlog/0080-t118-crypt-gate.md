# 0080 — M3 party-crypt gate: 5-person mixed-kit party reaches Gravemother (T-118)

T-118, bots-only gate leg. No server sim change, no epoch bump (20 stays), no wire change. The card asked: can a 5-person mixed-kit party (Ravager/Cultist/Gravecaller) form, traverse Thornwall → Thornwall Crypt → Drowned Crypt Depths, and fight Gravemother L14 (700 HP, Blood Bolt → telegraphed slam, Blood Curse)?
...
- `tools/t118_party_crypt_leg.sh` harness:
  - Fresh DB `/tmp/t118.db`, port 7817, record-world `logs/t118.bwj`.
  ...
  - Phase3: 300 s or 600 s crypt profile, prefix t118_.
...
2. 300 s, after fixing follower accept <5 and invite 40 tries: `CRYPT bossSeen=15 bossKills=2 curse=10 slam=28` — party 4, but **2 boss kills** by t118__00 (Ravager leader). TTK not directly measured, but kill occurred within 300 s window; boss is mathematically killable by L12 Pit Blade bots (20 dmg/hit vs 22 def, 700 HP → ~35 hits, 5-man focus ~7 hits each, CD 1.2 s). Curse and slam observed.
...
4. 600 s, same build: `CRYPT bossSeen=38 bossKills=0 curse=50 slam=86` — party 5 for all, kills 62, deaths 48, mend 52, mass mend 26, chorus 8, haste 15. Replay `ticks=12042 sessionCmds=4634 hashes=120 mismatches=0 entities=193`. This is the shipped `logs/t118.bwj`.
...
- **No epoch bump**: bots-only, no sim change. Suite 206/206, duel pin `b273be661b54673a`, replay t115 `ticks=202 mismatches=0`, replay t118 `ticks=12042 mismatches=0`.
...
- `bh_server --replay-world logs/t118.bwj`: OK ticks=12042 sessionCmds=4634 hashes=120 mismatches=0 entities=193
