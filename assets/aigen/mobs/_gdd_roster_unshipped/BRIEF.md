# GDD §11 roster not yet in kMobs (13 designs)

**Status:** design brief only — no art generated (volume art is gated on the B0 style-tile approval).
**Spec:** `docs/art/20-mobs.md § 'GDD-target roster'` (single source of truth for silhouette/palette/anims; do not fork it here).
**Cell / sheet:** 32×48 common / 40×60 elite / 48×64 Mud Golem / 64×64 Pale Sow · per class as above
**Palette family:** `per family (vermin, ghoul-flesh, hound, grave-goods, widow, choir-wax, night)` (shared strip; build with `bhpix.build_palette` over the whole family first)
...
SUBJECT = "see table in spec: Pale Cultist, Mine Wretch, Lantern Spider, Mud Golem, Crypt Revenant, Grave Banshee, Bell Ringer, Wraith, Bloodfiend, Old Maw, The Red Widow, Cantor Vex, The Pale Sow"
...
Produce only after the `kMobs` rows exist (ids TBD by the content owner); folders will be `mobs/<id>_<slug>/`.
